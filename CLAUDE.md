...
- **새로 쓰는 `.cpp`/`.h`는 UTF-8 with BOM, `.hlsl`/`.hlsli`는 BOM 없는 UTF-8.** `5a5f463` 커밋이 셰이더까지 BOM으로 통일하는 바람에 HLSL 컴파일러가 `X3000: Illegal character`로 컴파일에 항상 실패하는 회귀가 있었다(2026-09-13 발견·수정). 상세는 `MyPF/docs/CODEX_HANDOFF.md` §8 참고.
