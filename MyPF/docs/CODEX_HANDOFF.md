...
| 조명/Material | 약 60% | Directional/Ambient, 최대 8 Point Light, 거리 감쇠와 On/Off 완료. Emissive는 남음 |
| 실제 골목/상호작용 | 약 75% | PowerSwitch 컴포지션, 거리/시선/E 입력, On/Off 색상과 순차 점등 완료 |
| 고급 렌더링/연출 | 약 10% | Point Light 순차 점등 완료. Shadow/Wet/HDR/Bloom/Fog/Rain은 없음 |
...
- 브랜치: main
- 문서 갱신 기준 HEAD: ca6d15c — 순차점등시퀀스 적용 완료
...
- PointLightSequence가 1.4초 간격으로 생성 순서대로 켜고 역순으로 끔
...
7. PointLightSequence가 전원 상태를 목표로 받고 `deltaTime`을 누적해 Point Light를 한 단계 갱신한다.
...
- Point Light 점등 순서는 별도 연출 데이터가 아니라 Scene 생성 순서에 의존한다.
...
## 완료된 설계 방향: PointLightSequence

- AppBase가 값으로 소유하고 Scene을 비소유 참조한다.
- PowerSwitch, InputSystem, Camera, Renderer와 ImGui를 직접 알지 않는다.
- `Start(bool)`은 실제 조명을 즉시 바꾸지 않고 목표 개수와 진행 방향만 설정한다.
- `Update(deltaTime)`은 누적 시간이 간격에 도달할 때 Scene의 조명 하나만 변경한다.
- 켜기는 생성 순서, 끄기는 역순이며 진행 도중 목표가 바뀌면 현재 개수에서 방향을 전환한다.
- 초기화 시 내부 개수와 Scene의 실제 조명을 모두 꺼진 상태로 동기화한다.

## 바로 다음 작업

Emissive Material과 네온 표면 표현을 연결한다.
...
4. Pixel Shader가 방향광·점광원 결과와 별도로 Emissive를 최종 색상에 더한다.
5. 테스트 네온 오브젝트를 배치해 조명이 꺼져도 표면 자체가 밝게 보이는지 확인한다.
...
- 전원 On이면 생성 순서대로 켜고 Off이면 역순으로 끈다.
- 진행 중 방향 전환, 목표 도달 정지와 스위치 On/Off 연동을 실행 확인했다.
- 현재 간격은 1.4초이며 다음 기능은 Emissive Material과 네온 표면 표현이다.

## 노트북에서 바로 확인할 체크리스트

- git pull 후 HEAD가 최소 ca6d15c인지 확인한다.
...
- Point Light 4개가 1.4초 간격으로 켜지고 역순으로 꺼지는지 확인한다.
- 바로 다음 기능 책임은 Emissive Material이다.
