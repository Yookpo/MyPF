...
namespace My
{
	...
	{
	public:
		static bool CreateDepthBuffer(ID3D11Device* device, int screenWidth, int screenHeight,
		...
		static bool CreateVertexShaderAndInputLayout(ID3D11Device* device, const wstring& fileName,
		...
		static bool CreateVertexBuffer(
			ID3D11Device* device, const vector<T_VERTEX>& vertices, ComPtr<ID3D11Buffer>& vertexBuffer)
		{
		...
		static bool CreateConstantBuffer(
			ID3D11Device* device, const T_CONSTANT& constantBufferData, ComPtr<ID3D11Buffer>& constantBuffer)
		{
			static_assert((sizeof(T_CONSTANT) % 16) == 0, "Constant Buffer size must be 16-byte aligned");

			return CreateConstantBuffer(
				device, &constantBufferData, static_cast<uint32_t>(sizeof(T_CONSTANT)), constantBuffer);
		}
		...
		static bool UpdateBuffer(ID3D11DeviceContext* context, const T_DATA& bufferData, ID3D11Buffer* buffer)
		{
